...
// Достигли левой стенки, отскакиваем право
                ...
                if (x + speedX >= field.getWidth() - radius-200) {
                    if (enableMinimaze) {
                        radius -= minimazeRate;
                        if (radius <= 0)
                        {
...
// Достигли правой стенки, отскок влево
                    speedX = -speedX;
                    x = new Double(field.getWidth() - radius-200).intValue();
                } else
                if (y + speedY <= radius+200) {
                    if (enableMinimaze) {
                        radius -= minimazeRate;
                        if (radius <= 0)
                        {
                ...
                if (y + speedY >= field.getHeight() - radius-200) {
                    if (enableMinimaze) {
                        radius -= minimazeRate;
                        if (radius <= 0)
                        {
...
// Достигли нижней стенки
                    speedY = -speedY;
                    y = new Double(field.getHeight() - radius-200).intValue();
                } else {
// Просто смещаемся
                    x += speedX;
                    y += speedY;
